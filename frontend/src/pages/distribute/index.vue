...
async function loadData() {
  try {
    const [profile, statsData, userProfile] = await Promise.all([
      agentApi.my().catch(() => ({})),
      agentApi.stats().catch(() => ({})),
      api.get('/user/profile').catch(() => ({})),
    ]);
    // 合并：agent 基础资料 + user 资料（nickname/avatar）
    userInfo.value = {
      ...(profile.agent || {}),
      ...userProfile,
    };
    myCode.value = profile.agent?.promotion_code || profile.promotion_code || '';
    stats.value = statsData || profile.stats || {};
  } catch (e) {
