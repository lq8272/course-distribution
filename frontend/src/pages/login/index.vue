<template>
  <view class="login-page">
    ...
    <!-- 返回按钮 -->
    <view v-if="canGoBack" class="back-btn" @click="goBack">
...
// 仅当来源是 tabBar 页面或普通页面（非 reLaunch 到达的）才显示返回
if (prevPage) {
  const prevRoute = prevPage.route
  canGoBack.value = !['pages/index/index', 'pages/login/index'].includes('/' + prevRoute)
}
...
async function onGetPhone(e) {
  ...
  loading.value = true
  try {
    // 先用 wx.login() 获取登录态 js_code
    const loginRes = await new Promise((resolve) => {
      uni.login({
        provider: 'weixin',
        success: resolve,
        fail: () => resolve({ errMsg: 'fail' }),
      })
    })
    ...
    const data = await authApi.login(jsCode, '', promotionCode)
