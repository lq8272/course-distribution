<template>
  <view class="page">
    ...
    <view class="tabs">
      ...
      <view :class="tab === 'purchase' ? 'tab active' : 'tab'" @click="goPurchase">
        <text class="tab-icon">📦</text>
        <text class="tab-text">拿货</text>
      </view>
    </view>
    ...
    <block v-if="tab === 'course'">
      ...
      <view class="op-bar">
        ...
        <view class="btn-outline" style="margin-left:16rpx;" @click="openCategoryModal">分类管理</view>
      </view>
      ...
      <view class="modal modal-wide" @click.stop>
        ...
        <view class="form-item">
          ...
          <view class="cover-picker" @click="pickCover">
            <image v-if="formData.cover_image" :src="formData.cover_image" mode="aspectFill" class="cover-thumb" />
...
async function pickCover() {
  try {
    ...
    try {
      ...
      // 3. 只存 key，不存完整 URL（URL签名有时效，展示时动态获取签名URL）
...
async function saveCourse() {
  ...
  try {
    ...
    if (formData.value.id) {
      ...
      editingCourse.value = { id: result.id };
      // 刷新列表
